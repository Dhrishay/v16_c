...
class ResUsers(models.Model):
    ...
    def _login(cls, db, login, password, user_agent_env):
        ...
        try:
            with cls.pool.cursor() as cr:
                ...
                if user:
                    user_id = user.id
                    if not request.session.get("is_simulated"):
                        user.with_user(user_id)._check_credentials(password, user_agent_env)
    ...
    def name_search(self, name='', args=None, operator='ilike', limit=100):
        if self.env.context.get('user_simulation_context'):
            ...
            # args = args or []
            # args += [('id', 'in', user_ids)]
            domain = ['|', ('name', 'like', name), ('login', 'like', name)]
            user_ids = self.search(domain + args, limit=limit)
            args += [('id', 'in', user_ids.ids)]
