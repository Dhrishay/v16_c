                    ...
                    if ip_address in ip_list:
                        try:
                            ...
                            if e.args == odoo.exceptions.AccessDenied().args:
                                if not user_rec:
                                    values['login_error'] = _("Incorrect login. Please try again.")
                                else:
                                    values['pass_error'] = _("Incorrect password. Please try again")
                    else:
                        request.update_env = old_uid
                        values['error'] = _("Login from this IP is not permitted. Please contact your IT administrator for assistance.")
                else:
                    try:
                        ...
                        if sale_order:
                            if len(sale_order.order_line.ids) > 0 and not request.env.user._is_internal() or \
                                    request.params['redirect_to_add_to_cart'] == 'True':
